...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    //console.log(programmersList[featuredProgrammerId-1].name);
    if (featuredProgrammerId !== null)
      return programmersList[featuredProgrammerId - 1].name;
  };

  const style = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: featuredProgrammerId !== null ? "gold" : "royalblue", // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className="programmers">
        {
          ...
          programmersList.map((dev) => (
            <div className="programmer" key={dev.id}>
              ...
              <button
                onClick={() => {
                  featuredProgrammerId !== dev.id
                    ? setFeaturedProgrammerId(dev.id)
                    : setFeaturedProgrammerId(null);
                }}
              >
                Feature
              </button>
